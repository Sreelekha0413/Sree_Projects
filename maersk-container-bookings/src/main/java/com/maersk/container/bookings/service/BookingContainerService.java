...
public class BookingContainerService {
	...
	public BookingResponse bookContainer(BookingRequest bookingRequest) {
		/*
		 * Long recordCount = repo.count().block(); Mono<Bookings> booking =
		 * repo.save(dataConverter(bookingRequest, recordCount)); BookingResponse
		 * response = new BookingResponse(booking.block().getId());
		 */
		BookingResponse response = new BookingResponse(957000001L);
		return response;

	}

	/*
	 * private Bookings dataConverter(BookingRequest bookingRequest, Long
	 * recordCount) { Bookings bookings = new Bookings();
	 ...
	 * ); bookings.setId(957000001L + recordCount); return bookings; }
	 */
